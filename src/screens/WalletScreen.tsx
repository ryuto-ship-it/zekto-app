...
export default function WalletScreen() {
  ...
  const { purchased, redeemedCount, totalSaved } = useApp();
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {purchased.length === 0 ? (
        ...
        <View style={styles.list}>
          {purchased.map((p) => (
            <PassCard key={p.uid} pass={p} onPress={() => navigation.navigate('QRCode', { passUid: p.uid })} />
          ))}
        </View>
      )}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  stats: { flexDirection: 'row', gap: 10, marginHorizontal: 20, marginVertical: 14 },
  stat: { flex: 1, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: radii.lg, padding: 12 },
